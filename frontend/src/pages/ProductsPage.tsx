...
export default function ProductsPage() {
  ...
  async function startMachine() {
    ...
    try {
      ...
      if (res.data && typeof res.data.balance === 'number') {
        ...
        setMessage(
          `✅ Machine started! Your balance: $${res.data.balance.toFixed(2)}`
        )
      } else {
        setBalance(null)
  ...
  async function buyProduct(productId: string) {
    ...
    if (balance < cost) {
      setMessage(
        `❌ Insufficient funds! You have $${balance} but need $${cost}`
      )
      setPurchasing({})
      return
    }
    ...
    try {
      ...
      const quantity = quantities[productId] || 1
      const res = await api.post<PurchaseResponse>('/products/purchase', {
        productId,
        quantity,
      })
      ...
      if (res.data.success) {
        setMessage(
          `✅ Purchase successful! ${res.data.remaining || 0} items remaining.`
        )
        ...
        setMessage(
          `❌ Purchase failed: ${res.data.message || 'Purchase failed'}`
        )
      }
    } catch (e: any) {
      console.error(e)
      setMessage(
        `❌ Purchase failed: ${e.response?.data?.message || 'Request failed'}`
      )
    } finally {
    ...
    <div className='min-h-screen p-4 md:p-6 lg:p-8 pb-12'>
      <div className='max-w-7xl mx-auto'>
            ...
            {balance !== null && (
              <div className='px-6 py-4 bg-gradient-to-r from-green-500 via-emerald-500 to-teal-500 rounded-2xl shadow-2xl border-2 border-white/50 transform hover:scale-105 transition-transform'>
                <div className='flex items-center gap-3'>
                  ...
                  <span className='text-white text-3xl font-black text-shadow-lg'>
                    ${balance.toFixed(2)}
                  </span>
                </div>
              </div>
        ...
        {!loading && !error && products.length === 0 && (
          <div className='glass-card rounded-3xl shadow-2xl p-16 text-center border-2 border-gray-200'>
            ...
            <p className='text-gray-600 text-lg'>
              The backend might not be implemented yet.
            </p>
          </div>
        )}
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
          {[
            ...products,
            { id: '1', name: 'Not existing product', price: 0, stock: 3 },
          ].map((product) => (
            <ProductCard
              key={product.id}
              product={product}
