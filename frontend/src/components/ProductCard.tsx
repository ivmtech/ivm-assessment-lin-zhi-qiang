...
export default function ProductCard({
  ...
  return (
    <div key={product.id} className={`product-card border-gray-300`}>
      {/* Product Header with Icon */}
      <div
        className={`relative p-7 bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600`}
      >
        <div className='absolute top-4 right-4 bg-white/20 backdrop-blur-sm rounded-full p-2'>
          <span className='text-2xl'></span>
        </div>
        <h3 className='text-2xl font-black text-white text-shadow-lg pr-12'>
          {product.name}
        </h3>
      ...
      <div className='p-6 space-y-4'>
        ...
        <div className='grid grid-cols-2 gap-3'>
          ...
          <div
            className={`p-4 rounded-xl border-2 bg-gradient-to-br from-green-50 to-emerald-50 border-green-200`}
          >
            <div className='text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1'>
              Stock
            </div>
            <div className={`text-2xl font-black`}>{product.stock}</div>
          </div>
        </div>
        ...
        <div className='p-4 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 rounded-2xl shadow-lg'>
          <div className='flex justify-between items-center'>
            <span className='text-white/90 font-bold uppercase tracking-wide text-sm'>
              Total
            </span>
            <span className='text-3xl font-black text-white text-shadow-lg'>
              ${totalCost.toFixed(2)}
            </span>
          </div>
        </div>
        ...
        */}
        <button
          ...
          disabled={balance === null || product.stock <= 0}
          className={`w-full py-4 px-6 rounded-2xl font-black text-lg transition-all duration-300 shadow-xl hover:from-blue-700 hover:via-indigo-700 hover:to-purple-700 text-white hover:shadow-2xl hover:scale-105 active:scale-95} ${
            balance === null || product.stock <= 0
              ? 'text-gray-400 bg-gray-300 hover:cursor-not-allowed'
